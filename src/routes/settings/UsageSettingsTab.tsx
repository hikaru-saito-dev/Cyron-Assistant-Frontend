...
export const UsageTab = ({
    ...
    return (
        <motion.div key="usage" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }} className="space-y-4">
            <div>
                <TextBlurIn className="text-white font-bold uppercase text-[2.5rem] md:text-[3.5rem] leading-[0.85] tracking-tighter font-display">Usage Analytics</TextBlurIn>
            ...
            {loading && (
                <div className="cyron-glass cyron-glass-hover p-5">
            ...
            {usage && !loading && (
                <>
                    <motion.div initial={{ opacity: 0, filter: "blur(10px)", y: 10 }} animate={{ opacity: 1, filter: "blur(0px)", y: 0 }} transition={{ duration: 0.6, delay: 0.3 }} className="grid gap-4 grid-cols-1 sm:grid-cols-3">
                        {[
                            ...
                            <motion.div key={label} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                                className="cyron-glass cyron-glass-hover p-5">
                                <p className="text-[12px] font-bold uppercase tracking-wider text-slate-500">{label}</p>
                                <p className="mt-2 text-2xl font-bold text-[#F5A623]">
                                ...
                                <div className="mt-4 h-1.5 w-full rounded-full bg-white/5 overflow-hidden">
                                    <motion.div initial={{ width: 0 }} animate={{ width: `${Math.min(100, (usedN / limitN) * 100)}%` }}
                                        transition={{ duration: 0.4 }} className="h-full rounded-full bg-[#F5A623]" />
                                </div>
                            </motion.div>
                            );
                        })}
                    ...
                    <motion.div initial={{ opacity: 0, filter: "blur(10px)", y: 10 }} animate={{ opacity: 1, filter: "blur(0px)", y: 0 }} transition={{ duration: 0.6, delay: 0.4 }} className="cyron-glass cyron-glass-hover p-5">
                        ...
                        <div className="h-48 w-full min-h-[12rem]">
                            <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={180}>
                                <AreaChart data={chartData.length ? chartData : [{ date: '—', tokens: 0 }]} margin={{ top: 4, right: 4, left: -20, bottom: 0 }}>
                                    ...
                                    <Area type="monotone" dataKey="tokens" stroke="#F5A623" strokeWidth={2} fill="url(#tg)" />
                                </AreaChart>
                            </ResponsiveContainer>
                        </div>
                    </motion.div>

                    <motion.div initial={{ opacity: 0, filter: "blur(10px)", y: 10 }} animate={{ opacity: 1, filter: "blur(0px)", y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="cyron-glass cyron-glass-hover p-5">
                            ...
                            {recentActivity.map((row) => (
                                <div key={row.id} className="rounded-xl border border-white/10 bg-white/5 p-4 text-[13px]">
                                    <div className="flex items-center justify-between">
                                        <span className="font-mono font-bold text-[#F5A623]">{row.tokens} tokens</span>
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {recentActivity.map((row) => (
                                        <tr key={row.id} className="transition-colors hover:bg-white/5">
                                            ...
                                            <td className="px-4 py-3 font-mono font-bold text-[#F5A623]">{row.tokens}</td>
